...
def process_sent_word(sentence):
    ...
    segmentor.release()
    return list(words)
def process_sent_char(sentence_tokens):
    all_tokens = []
    for word in sentence_tokens:
        all_tokens.append(list(word))
    return all_tokens
def process_sent_spans(sentence_tokens):
    start_id = 0
    end_id = 0
    spans_tmp = []
    for word in sentence_tokens:
        end_id += len(word)
        spans_tmp.append((start_id,end_id))
        start_id += len(word)
    return spans_tmp
def process_file(filename, data_type, word_counter, char_counter):
    ...
    with open(filename,"r") as fh:
        source = json.load(fh)
        # print(source[0])
        # context_text,qas,title
        # context_tokens,context_chars,ques_tokens,ques_chars,y1s,y2s,context_id,query_id
        for article in tqdm(source):
            context_text = article['context_text']
            context_tokens = process_sent_word(article['context_text'])
            context_chars = process_sent_char(context_tokens)
            ...
            questions = []
            spans = []
            for qas_a in article['qas']:
                ques_tokens = process_sent_word(qas_a['query_text'])
                ques_chars = process_sent_char(ques_tokens)
                query_id = qas_a['query_id']
                answer = str(qas_a['answers'][0])
                Length = len(answer)
                y1s = context_text.find(answer)
                y2s = y1s + Length
                ...
                spans = process_sent_spans(context_tokens)
                eval_examples[str(total)] = {"context": context_text, "spans": spans, "answers": [answer], "context_id": context_id,"query_id":query_id}
                total += 1
